...
def liquibase_install():
    ...
    with cd(settings['working_directory']):
        if install_liquibase:
            liquibase_url = settings['liquibase']['url'] % (
                settings['liquibase']['version'], settings['liquibase']['version'])
            postgres_driver_url = settings['postgres']['driver.url'] % (
                settings['postgres']['driver.version'], settings['postgres']['driver.version'])
...
def schemacrawler_install():
    ...
    with cd(settings['working_directory']):
        schemacrawler_url = settings['schemacrawler']['url'] % (
            settings['schemacrawler']['version'], settings['schemacrawler']['version'])
...
def apache_tomcat_install():
    ...
    with cd(settings['working_directory']):
        ...
        postgres_driver_url = settings['driver']['postgres.url'] % (
            settings['driver']['postgres.version'], settings['driver']['postgres.version'])
        h2_driver_url = settings['driver']['h2.url'] % (
            settings['driver']['h2.version'], settings['driver']['h2.version'])
        javax_mail_url = settings['javax.mail']['url'] % (
            settings['javax.mail']['version'], settings['javax.mail']['version'])
        jt400_url = settings['driver']['jt400.url'] % (
            settings['driver']['jt400.version'], settings['driver']['jt400.version'])
        activation_url = settings['activation']['url'] % (
            settings['activation']['version'], settings['activation']['version'])
...
def edit_oh_my_zshrc():
    with cd("$HOME"):
        ...
        # Add our own alias
        # java
        append_to_file("## PATH", zshrc_file)
        ...
        # append_to_file("export JAVA_OPTS=\"-Xms1024m -Xmx20000m\"", zshrc_file)
